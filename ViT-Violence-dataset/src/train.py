...
i have to refactor this code due to poor performance , use with caution.
"""
...
class ViolenceDetectionTrainer:
    ...
    def __init__(
        ...
        self.scaler = GradScaler(device_type='cuda') if self.device.type == 'cuda' else None  # Updated for torch.amp and device-aware
        # Metrics tracking
        self.train_losses = []
        self.val_losses = []
    ...
    def train_epoch(self, epoch):
        ...
        for videos, targets in pbar:
            ...
            # Use autocast only for CUDA, otherwise use nullcontext
            autocast_ctx = autocast(device_type='cuda') if self.device.type == 'cuda' else nullcontext()
            with autocast_ctx:
                outputs = self.model(videos)
                loss = self.criterion(outputs, targets)
            # Use scaler for CUDA, otherwise regular backward/step
...
def main():
    ...
    CONFIG = {
        'data_path': 'dataset',
        'batch_size': 4,  # Reduced for M4 MacBook Air
        'sequence_length': 16,
        'img_size': 224,
        'num_epochs': 50,
        'learning_rate': 1e-4,  # Slightly lower for stability
        'weight_decay': 1e-4,
        'num_workers': 8,  # Use half of CPU cores, min 1
    ...
    pin_memory = device.type == 'mps'  # Only enable pin_memory for CUDA
